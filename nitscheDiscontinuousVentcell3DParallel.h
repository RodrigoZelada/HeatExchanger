void nitscheDiscontinuousVentcell3D(Mat A, Vec b, mesh3D &Th, vector<double> &kappa, vector<vector<double>> &u, map<int,int> Gamma, mesh3D &Th2, DirichletBC &DBC1, DirichletBC &DBC2, PetscInt Istart, PetscInt Iend)
{
	int N = Th.ntet;
	int Nt = Th.nt;
	...
	for (n = 0; n < N; n++){
		...
		kRegion = k1; rhocp = rhocp1; 
		if (region == region2){kRegion = k2; rhocp = rhocp2;}
		...
		for (i = 0; i <= D; i++){
			...
			if ((ii_global >= Istart) && (ii_global < Iend)){
				for (j = 0; j <= D; j++){
					...
					if ( (iibelongDBC == 0) && (jjbelongDBC==3) ){ //if j is Dirichlet
