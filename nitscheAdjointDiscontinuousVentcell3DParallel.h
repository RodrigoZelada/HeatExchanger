void nitscheAdjointDiscontinuousVentcell3D(Mat A, Vec b, mesh3D &Th, vector<double> &kappa, vector<vector<double>> &u, map<int,int> Gamma, mesh3D &Th2, DirichletBC &DBC1, DirichletBC &DBC2, PetscInt Istart, PetscInt Iend)
{
	int N = Th.ntet;
	int Nt = Th.nt;
	...
	for (n = 0; n < N; n++){
		...
		kRegion = k1; rhocp=rhocp1; sign = 1.;
		if (region == region2){kRegion = k2; rhocp=rhocp2; sign=-1.;}
		...
		for (i = 0; i <= D; i++){
			...
			if ((ii_global >= Istart) && (ii_global < Iend) && (iibelongDBC==0)){
				val = 0.;
				for (int p=0; p<=D; p++){
					int pp=Th.tetrahedra[n][p];
					vector<double> up = {u[pp][0],u[pp][1],u[pp][2]};
					val += sign*rhocp*(volume/4.)*dot(gradii,up); 
				}
				...
				for (j = 0; j <= D; j++){
					...
					for (int p=0; p<=D; p++){
						...
						vector<double> up = {u[pp][0],u[pp][1],u[pp][2]};
						val -= rhocp*(volume/20.)*dot(coefjj,xp1)*dot(gradii,up);
						...
						for (int q=0; q<p; q++){
							...
							val += rhocp*(volume/5.)*dot(coefjj,xpq1)*dot(gradii, upq);
    ...
    for (int t=0; t<Nt; t++){
		...
		switch (label_triangle)
		{
			...
			case 10: //label_interface
				if (region == region1){ 
					normal[0]=normalOmegai[0];normal[1]=normalOmegai[1];normal[2]=normalOmegai[2];
					sign = 1.; kRegion=k1; kOtherRegion=k2;
				}
				else{
					normal[0]=-normalOmegai[0];normal[1]=-normalOmegai[1];normal[2]=-normalOmegai[2];
					sign = -1.; kRegion=k2; kOtherRegion=k1;
				}
							...
							if ( jjbelongDBC==0){
								...
								for (int p=0; p<D; p++){
									...
									for (int q=0; q<p; q++){
										...
										//- int_F (be/eta)*ks*H*[T*S] = - int_F ce*ks*H*(T1*S1 - T2*S2 )
										//= -ce*ks*H*T1*S1 // ce*ks*H*T2*S2
										val -= sign*(area/3.)*ce*ks*Hpq*dot(coefii,xpq1)*dot(coefjj,xpq1);

										//-int_F be*ks*H*<T>*<kgradN(S)> + be*ks*H*<S>*<kgradN(T)>
										val -= 0.25*be*ks*Hpq*(area/3.)*(kRegion*dot(gradjj,normal) *  dot(coefii,xpq1) ) + 0.25*be*ks*Hpq*(area/3.)*(kRegion*dot(gradii,normal) *  dot(coefjj,xpq1) );

										//-int_F be*ks^2*Hpq^2*<T><S>
										val -= 0.25*be*ks*ks*Hpq*Hpq*(area/3.)*dot(coefii,xpq1)*dot(coefjj,xpq1);
									}
								}
							...
							if (jjbelongDBC==0){
								...
								for (int p=0; p<D; p++){
									...
									for (int q=0; q<p; q++){
										...
										//- int_F (be/eta)*ks*H*[T*S] = - int_F ce*ks*H*(T1*S1 - T2*S2 )
										//= -ce*ks*H*T1*S1 // ce*ks*H*T2*S2
										//val += sign*(area/3.)*ce*ks*Hpq*dot(coefii,xpq1)*dot(coefjj_tilde,xpq1);

										//-int_F be*ks*H*<T>*<kgradN(S)> + be*ks*H*<S>*<kgradN(T)>
										val -= 0.25*be*ks*Hpq*(area/3.)*(kOtherRegion*dot(gradjj_tilde,normal) *  dot(coefii,xpq1) ) + 0.25*be*ks*Hpq*(area/3.)*(kRegion*dot(gradii,normal) *  dot(coefjj_tilde,xpq1) );

										//-int_F be*ks^2*Hpq^2*<T><S>
										val -= 0.25*be*ks*ks*Hpq*Hpq*(area/3.)*dot(coefii,xpq1)*dot(coefjj_tilde,xpq1);
									}
								}
