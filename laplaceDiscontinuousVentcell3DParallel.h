void laplaceDiscontinuousVentcell3D(Mat A, Vec b, mesh3D &Th, vector<double> &kappa, vector<vector<double>> &u, map<int,int> Gamma, mesh3D &Th2, DirichletBC &DBC1, DirichletBC &DBC2, PetscInt Istart, PetscInt Iend)
{
	int N = Th.ntet;
	int Nt = Th.nt;
				...
				for (i=0; i<D; i++){
					...
					if ((ii_global >= Istart) && (ii_global < Iend) ){
						for (j=0; j<D; j++){
							...
							if ( (iibelongDBC == 0) && (jjbelongDBC_tilde==3) ){ //if j is Dirichlet
								val=-val*T1(Th.vertices[jj][0],Th.vertices[jj][1],Th.vertices[jj][2]); //T2=0
								VecSetValues(b,1,&ii_global,&val,ADD_VALUES);
							}
