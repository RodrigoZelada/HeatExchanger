...
output='output/case1' #1
import os
os.makedirs(output,exist_ok=True)

config={'OUTPUT':output}

N=64 #8 #number of cores
i = format(0,'04d')
meshes='CubeTube'
Th0 = Mesh3D(meshes+"/CubeCylinderCoarse.mesh")
...
phi1 = lambda x,y,z: (x-0.5)**2 + (y-0.5)**2 - r1**2
...
refermmg = f"""
                ...
                10 Triangles 5e-3 1e-2 5e-3
                1 Triangles 1e-2 2.5e-2 1e-3
                2 Triangles 1e-2 2.5e-2 1e-3
                3 Triangles 5e-3 2e-2 1e-3
                4 Triangles 5e-3 2e-2 1e-3
"""
...
real maxVolHot =  0.19390603522221508; //pi*0.25**2*1
real maxVolCold = 1.-maxVolHot;
...
def computeU(mesh):
    ...
    os.system(f"ff-mpirun -np {N} NSColdCylinder.edp -output {output} -v 0")
    os.system(f"ff-mpirun -np {N} NSHotCylinder.edp -output {output} -v 0")
...
real zcen = 0.5; real ymax = 0.5; real ymin = 0.5; real zmin=0.05; real zmax=1.-zmin; real r=0.25;
